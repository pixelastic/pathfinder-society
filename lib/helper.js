...
const cache = require('firost/lib/cache');
...
module.exports = {
   ...
   **/
  allLocations() {
    if (cache.has(this.locationCacheKey)) {
      return cache.read(this.locationCacheKey);
    }
    ...
    const allLocations = _.chain(this.__data())
      ...
      .sort()
      .value();
    cache.write(this.locationCacheKey, allLocations);
    return allLocations;
  },
  /**
   ...
   **/
  isReleased(wikiPage) {
    const firstParagraph = wikiPage.doc().paragraphs()[0].text();
  ...
  async paizoPage(websiteUrl) {
    ...
    const basename = urlToFilepath(websiteUrl);
    const cachePath = path.resolve(this.cacheLocation, 'paizo', basename);
    let htmlContent;
    if (await exists(cachePath)) {
      htmlContent = await read(cachePath);
    } else {
      htmlContent = await download(websiteUrl, cachePath);
    }
  ...
  npcs(wikiPage) {
    ...
    return _.chain(mergedList).difference(allLocations).sort().value();
  },
  /**
  ...
  rating(paizoPage) {
    ...
    return _.chain(value).split('/').first().thru(parseFloat).value();
  },
  /**
